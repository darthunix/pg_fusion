...
impl<'a> BlockRef<'a> {
    ...
    pub fn fixed_value(&self, index: usize, row: u32) -> Result<&[u8], LayoutError> {
        ...
        let end = start.checked_add(row_width).ok_or(LayoutError::SizeOverflow)?;
...
impl<'a> BlockMut<'a> {
    ...
    pub fn write_view_bytes(
        ...
        self.set_validity(index, row, true)?;
        self.write_view(index, row, ByteView::new_outline(bytes, start - self.pool_base())?)?;
    ...
    pub fn write_view(&mut self, index: usize, row: u32, view: ByteView) -> Result<(), LayoutError> {
        let layout = self.column_layout(index)?;
        let type_tag = layout.type_tag;
        if !type_tag.is_view() {
            return Err(LayoutError::InconsistentViewFlag {
    ...
    pub(crate) fn tail_bytes_mut(&mut self, start: u32, len: u32) -> Result<&mut [u8], LayoutError> {
    ...
    fn zero_value_slot(&mut self, index: usize, row: u32, row_width: usize) -> Result<(), LayoutError> {
        let layout = self.column_layout(index)?;
        let width = layout
            .type_tag
            .values_row_width()
