...
scan timing is enabled. `scan_arrow_encode_ns` isolates row callback time and is
split further by `scan_append_precheck_ns`, `scan_tupledesc_check_ns`,
`scan_slot_deform_ns`, `scan_cell_extract_ns`, `scan_varlena_detoast_ns`,
`scan_page_write_ns`, `scan_row_encode_outer_ns`,
`scan_append_status_ns`, and `scan_encode_unclassified_ns`. If the benchmark
ratio is poor but those
counters are small, inspect `scan_b2w_wait_ns`, `scan_batch_send_ns`,
...
To isolate the Rust page encoding work behind `scan_arrow_encode_ns`, run the
PostgreSQL-free Criterion benchmark:
...
`arrow_layout` pages. It corresponds most closely to `scan_page_write_ns`; the
remaining `scan_arrow_encode_ns` buckets cover PostgreSQL slot deformation,
datum extraction, detoasting, tuple descriptor checks, row-encoder outer
bookkeeping, append status handling, and callback glue. If the CSV directory
is absent, it falls back to a deterministic q05-like synthetic fixture so the
benchmark target still compiles and runs without PostgreSQL.
