...
the completed build set. It also models the shared-memory lease rule that only
one builder owns a payload at a time; acquiring a new builder clears payload
only after the slot has moved from `Free` or `Disabled` to `Building`. Ready
retirement is modeled as an external quiescence boundary: the implementation
exposes it as unsafe because active probes cannot be tracked by the Bloom bits
themselves.
