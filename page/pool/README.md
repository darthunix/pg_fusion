...
`pool` is a standalone workspace crate for fixed-size page ownership inside a caller-provided shared-memory region.
...
2. `pool` manages which process owns which page.
...
```sh
cargo test -p pool
cargo clippy -p pool --tests -- -D warnings
cargo doc -p pool --no-deps
```
