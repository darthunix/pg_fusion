...
`transfer` is a standalone workspace crate for transferring ownership of shared-memory pages managed by `pool`.
...
- A sender/receiver layer over `pool`.
...
Because `payload_len` is encoded as `u32`, `transfer` rejects page configurations whose payload capacity exceeds `u32::MAX`.

## Sync and async use

`transfer` stays runtime-agnostic:
...
use transfer::{encode_frame, FrameDecoder, PageRx, PageTx, ReceiveEvent};
...
cargo clippy -p transfer --tests -- -D warnings
cargo doc -p transfer --no-deps
```
