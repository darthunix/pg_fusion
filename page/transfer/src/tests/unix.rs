...
impl SharedFileRegion {
    fn create(len: usize) -> Self {
        let unique = SystemTime::now()
            ...
            .expect("clock drift")
            .as_nanos();
        let path = env::temp_dir().join(format!("transfer_test_{}_{}", std::process::id(), unique));
...
fn multiprocess_child_entry() {
    ...
    let pool = unsafe { pool::PagePool::attach(region.base, region.len) }.expect("attach");
...
fn multiprocess_handoff_roundtrip() {
    ...
    let layout = pool::PagePool::layout(config).expect("layout");
    let region = SharedFileRegion::create(layout.size);
    let pool =
        unsafe { pool::PagePool::init_in_place(region.base, region.len, config) }.expect("init");
