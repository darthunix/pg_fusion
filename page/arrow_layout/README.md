...
- front-region offsets use a fixed `BUFFER_ALIGNMENT_BIAS = 12`, so when the block lives inside a `transfer` payload with its current 20-byte in-page header, 8-byte and 16-byte Arrow buffers are still physically aligned for zero-copy import
...
such as `import` use `BlockRef` to validate and slice page-backed buffers
without copying.
